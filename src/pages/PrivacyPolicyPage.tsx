  ...
  useEffect(() => {
    const fetchPrivacyPolicy = async () => {
      try {
        ...
        setError('Error al cargar la política de privacidad. Por favor, inténtalo de nuevo más tarde.');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="container mx-auto px-4 py-12">
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-6 md:p-8 max-w-4xl mx-auto">
            <h1 className="text-3xl font-bold text-primario mb-6">{policyData.title}</h1>
            
            <div className="text-sm text-gray-500 mb-6 flex flex-wrap gap-x-6">
              <p>Fecha de publicación: {formatDate(policyData.date)}</p>
              <p>Última actualización: {formatDate(policyData.modified)}</p>
            </div>
          ...
          <div className="text-center p-8">
            <p className="text-lg text-gray-600">No se encontró la política de privacidad</p>
          </div>
        )}
      </div>
    </div>
